...
export default function InsightStrip({ items = [], loading = false }) {
  ...
  const copyAll = async () => {
    try {
      await navigator.clipboard.writeText(texts.map((t) => `• ${t}`).join('\n'));
  ...
  return (
    <Card
      title="Auto-insights"
      subtitle="Generated from the filtered data on every change"
      actions={(
        <div className="flex items-center gap-2">
          <Badge tone="slate">rule-based · no LLM</Badge>
          {texts.length > 0 && (
            <Tooltip label="Copy all insights">
              <button type="button" className="btn-ghost !px-2" onClick={copyAll} aria-label="Copy all insights">
    ...
    >
      {loading ? (
        ...
        <p className="text-xs text-muted">Insights appear once the charts below have data for the current filters.</p>
      ) : (
