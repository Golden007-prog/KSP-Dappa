  ...
  return (
    <section className="min-w-0 rounded-xl border border-grid bg-base/40 p-3" aria-label={t('copilot.compare.paneAria', { side: t(`copilot.compare.${side}`) })}>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        {conf && <Badge tone={conf.tone}>{t(`copilot.conf.${conf.level}.label`)}</Badge>}
...
export default function CompareSheet({ open, onClose, left, right, onSwap }) {
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={t('copilot.compare.sheetTitle')}
      className="md:!w-[60rem] md:!max-w-[calc(100vw-4rem)]"
    >
      <div className="flex items-center justify-between gap-2 mb-2 px-0.5">
        <p className="text-[11px] text-muted">{t('copilot.compare.sheetHint')}</p>
        <button
          type="button"
          className="btn-ghost !px-2 !text-xs min-h-[40px] shrink-0"
          onClick={onSwap}
          aria-label={t('copilot.compare.swapAria')}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M16 3h5v5M21 3l-7 7M8 21H3v-5M3 21l7-7" />
          </svg>
          {t('copilot.compare.swap')}
        </button>
      </div>
