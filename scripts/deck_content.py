...
def _caps():
    """Capability counts for slide 4, read from docs/capability_counts.json
    (derived from the FEATURES.md tags by scripts/check_capabilities.mjs) so a
    slide can never disagree with CAPABILITIES.md or the README."""
    import json
    import pathlib
    path = pathlib.Path(__file__).resolve().parent.parent / "docs" / "capability_counts.json"
    return json.loads(path.read_text(encoding="utf-8"))


def _cap_headline():
    c = _caps()
    passing = [k for k in ("C1", "C2", "C3", "C4", "C5", "C6") if c["counts"][k] >= c["bar"]]
    short = [k for k in ("C1", "C2", "C3", "C4", "C5", "C6") if c["counts"][k] < c["bar"]]
    verdict = (f"{len(passing)} of 6 clear the 100-feature bar under the Round-2 rules; "
               + (", ".join(f"{k} {c['counts'][k]}" for k in short) + " fall short and say so.")
               if short else "all six clear the 100-feature bar.")
    return (f"{c['total']:,} features catalogued and individually tagged (FEATURES.md); "
            f"{c['scored']} carry analytic or visual substance, {c['infra']} are infrastructure and are excluded. {verdict}")


def _cap_line(key, text):
    return f"{text} ({_caps()['counts'][key]})"


def _services_line():
    """Slide-9 headline, read from docs/service_table.json (generated by
    scripts/gen_service_table.mjs from servicemap.js) so the deck can never
    disagree with the README or the code."""
    import json
    import pathlib
    path = pathlib.Path(__file__).resolve().parent.parent / "docs" / "service_table.json"
    svc = json.loads(path.read_text(encoding="utf-8"))
    s = svc["summary"]
    by = s["byStatus"]
    return (
        f"GET /meta/services maps {s['total']} Catalyst services, {s['reachableFromCode']} with a real call site; "
        f"{by.get('live', 0)} live, {by.get('active', 0)} active behind a flag, {by.get('platform', 0)} platform, "
        f"{by.get('console-pending', 0)} waiting on a named console step."
    )
...
SLIDES = [
    {
        ...
        "bullets": [
            ...
            "Bilingual interface — English and Kannada — switchable at any time, rendered with system fonts so nothing is fetched from outside Catalyst.",
    ...
    },
    {
        ...
        "bullets": [
            _cap_headline(),
            _cap_line("C1", "Advanced visualisation: choropleth, incident heat, Gi* hotspots, patrol routing, hour×weekday seasonality, brushed 12-month trends, CB-safe palette"),
            _cap_line("C2", "Network & link analysis: co-accused graph over 23,833 edges, shortest path between two persons, community detection, victim and recurring-location projections"),
            _cap_line("C3", "Predictive dashboards: 30-day risk score for all 282 stations with named drivers, Holt-Winters forecasts with 80% intervals and backtest MAPE"),
            _cap_line("C4", "Pattern & trend discovery: auto-insights recomputed on every filter change, emerging-category watch, z-score anomaly feed with SLA ageing"),
            _cap_line("C5", "Behavioural analysis: crews from shared rare MO, MO evolution, escalation signals, offender peer cohorts") + " · " + _cap_line("C6", "AI/ML: Zia OCR/NER, QuickML risk model, copilot with auditable ZCQL"),
    ...
    },
    {
        ...
        "bullets": [
            ...
            "i18n: 12 namespaces per locale, ~5,300 keys each for English and Kannada, loaded eagerly via import.meta.glob; system fonts only, zero external requests.",
    ...
    },
    {
        ...
        "images": ["07-predict-risk.jpg", "05-offenders.jpg",
                   ...
                   "12-dashboard-kn.jpg", "08-cases.jpg"],
        "caption": "Predict · Offenders · Ask DAPPA · Alerts · the same dashboard in Kannada · Case Explorer — all captured from the live deployment.",
    },
    {
        "slideNumber": 12,
        "bullets": [
